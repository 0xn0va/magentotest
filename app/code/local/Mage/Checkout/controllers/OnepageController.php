    ...
    public function saveShippingMethodAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!$result) {
                ...
                //add check for account manager special order availability


                foreach (range(1, 21) as $ukPostNumber) {
                    $ukPostalSector = "GU$ukPostNumber";
                    echo "$ukPostalSector";
                }

                $specialThreshold = Mage::getModel('core/variable')->loadByCode('customerServiceThreshold')->getValue('plain');
                $specialOrder = Mage::getSingleton('checkout/session')->getQuote();

                if ($specialOrder >= $specialThreshold) {
                    $postalsector = $observer->getEvent()->getOrder()->getShippingAddress();
                    $postalsector->getData('postcode');
                    if ($postalsector == $ukPostalSector) {
                        $collection->getSelect()->join('abdo_vaimo_accmanagers', 'sales_flat_order_address.postcode = abdo_vaimo_accmanagers.postal_sector',array('name'));
